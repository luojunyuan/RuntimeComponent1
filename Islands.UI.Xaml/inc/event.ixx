...
inline std::int64_t s_eventHandlerId = 0;

template <typename ImplT, typename T, typename StorageT = T>
class event_base
{
protected:
    std::shared_ptr<std::map<std::int64_t, StorageT>> m_handlers;
...
public:
    ...
    winrt::event_token add(const T & value)
    {
        ...
        auto handlers = std::make_shared<std::map<std::int64_t, StorageT>>();
