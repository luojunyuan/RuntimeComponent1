...
export module inc.SharedHelpers;

import std;
import inc.common_base;
import inc.AutoHandle;
...
public:
    ...
    {
        MUXControls::Common::Handle synchronizationHandle(createEvent(nullptr, win32False, win32False, nullptr));
            ...
            [&synchronizationHandle](winrt::IAsyncAction asyncAction, winrt::AsyncStatus asyncStatus)
        {
            ...
            {
                throw winrt::hresult_error(eFail, L"Async operation failed!");
            }
        }));

        waitForSingleObject(synchronizationHandle, infiniteTimeout);
    }
    ...
    static winrt::InMemoryRandomAccessStream CreateStreamFromBytes(const winrt::array_view<const std::byte>& bytes);
    ...
    static void ClampCornerRadii(float radius1, float radius2, float edgeLength, float* pRadius1Clamped, float* pRadius2Clamped);
