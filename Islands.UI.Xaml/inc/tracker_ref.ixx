...
export module inc.tracker_ref;

import std;
import inc.common_base;
import inc.StaticAssertFalse;
...
struct __declspec(novtable) ITrackerHandleManager
{
    ...
    bool GetTrackerValue(TrackerHandle handle, IUnknown** value) const try
    {
#ifdef DBG
        MUX_ASSERT_NOASSUME(m_wasEnsureCalled);
#endif
...
struct IUnknownAccessor
{
    ...
    static winrt::com_ptr<IUnknown> const& get(T const& value)
    {
        // This is known to be an IUnknown already, so reinterpret it
        return reinterpret_cast<winrt::com_ptr<IUnknown> const&>(value);
    }
};

template <typename T>
struct IUnknownAccessor<winrt::com_ptr<T>>
{
    static winrt::com_ptr<IUnknown> get(winrt::com_ptr<T> const& value)
    {
...
public:
    ...
    explicit tracker_ref(std::nullptr_t) = delete;
...
template<typename T>
using tracker_com_ref = tracker_ref<winrt::com_ptr<T>, TrackerRefFallback::None, T*>;
