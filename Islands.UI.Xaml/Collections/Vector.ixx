...
export module ixx.Vector;

import std;
import inc.common;
import inc.RuntimeClassHelpers;
import inc.tracker_ref;
import ixx.VectorIterator;
import ixx.VectorChangedEventArgs;
...
struct ObservableTraits<T, true, isBindable>
{
    ...
    static void RaiseEvent(EventSource* e, SenderType sender, winrt::CollectionChange collectionChange, std::uint32_t index)
    {
...
struct IVectorOwner
{
    ...
    virtual std::function<bool(T const& value, std::uint32_t& index)> GetCustomIndexOfFunction() { return nullptr; };
};
...
private:
    ...
    typename T_type GetAt(std::uint32_t const index, bool useSafeGet)
    {
        if (index < static_cast<std::uint32_t>(m_vector.size()))
        {
...
public:
    ...
    typename T_type GetAt(std::uint32_t const index)
    {
        return GetAt(index, false);
    }

    typename T_type SafeGetAt(std::uint32_t const index)
    {
        return GetAt(index, true);
    }

    void SetAt(std::uint32_t const index, typename T_type const& value)
    {
        if (index < static_cast<std::uint32_t>(m_vector.size()))
        {
    ...
    void Append(typename T_type const& value)
    {
        ...
        RaiseChildrenChanged(winrt::CollectionChange::ItemInserted, static_cast<std::uint32_t>(m_vector.size()) - 1);
    }

    bool IndexOf(typename T_type const& value, std::uint32_t& index)
    {
        index = 0;
        ...
        if (it != m_vector.end())
        {
            index = (std::uint32_t)(it - m_vector.begin());
            return true;
        }
        return false;
    }

    bool IndexOf_OptimizedForRemove(typename T_type const& value, std::uint32_t& index)
    {
        index = 0;
    ...
    std::uint32_t GetMany(std::uint32_t const startIndex, winrt::array_view<T_type> values)
    {
        ...
        std::uint32_t actual = static_cast<std::uint32_t>(m_vector.size() - startIndex);
    ...
    void InsertAt(std::uint32_t const index, typename T_type const& value)
    {
        if (index <= static_cast<std::uint32_t>(m_vector.size()))
        {
    ...
    void RemoveAtEnd()
    {
        ...
        {
            m_vector.pop_back();
            RaiseChildrenChanged(winrt::CollectionChange::ItemRemoved, static_cast<std::uint32_t>(m_vector.size()));
        }
    }
...
#define Implement_IVector_Read_Functions(Options) \
    public: \
        ...
        typename Options##::T_type GetAt(std::uint32_t index) \
        { \
            auto inner = this->GetVectorInnerImpl(); \
            return inner->GetAt(index); \
        } \
        bool IndexOf(typename Options##::T_type const& value, std::uint32_t& index) \
        { \
        ...
        std::uint32_t GetMany(std::uint32_t const startIndex, winrt::array_view<typename Options##::T_type> values) \
        { \
            auto inner = this->GetVectorInnerImpl(); \
...
#define Implement_IVector_Modify_Functions(Options) \
    public: \
        void SetAt(std::uint32_t const index, typename Options##::T_type const& value) \
        { \
            auto inner = this->GetVectorInnerImpl(); \
        ...
        void InsertAt(std::uint32_t index, typename Options##::T_type const& value) \
        { \
            auto inner = this->GetVectorInnerImpl(); \
...
public:
    ...
    VectorBase(std::uint32_t capacity) : VectorBase()
    {
        GetVectorInnerImpl()->reserve(capacity);
    }

protected:
    void SetCustomIndexOfFunction(std::function<bool(T const& value, std::uint32_t& index)> indexOfFunction)
    {
        m_indexOfFunction = indexOfFunction;
    }
    virtual std::function<bool(T const& value, std::uint32_t& index)> GetCustomIndexOfFunction() { return m_indexOfFunction; };
private:
    std::function<bool(T const& value, std::uint32_t& index)> m_indexOfFunction{ };
};
...
public:
    Vector() {}
    Vector(std::uint32_t capacity) : VectorBase<T, Helper::isObservable, Helper::isBindable, Helper::isDependencyObjectBase, Helper::isNoTrackerRef>(capacity) {}
    ...
    Vector(std::function<int(typename T const& value)> indexOfFunction) : m_indexOfFunction(indexOfFunction)
    {
        if (m_indexOfFunction)
        {
            this->SetCustomIndexOfFunction(
                [this](T const& value, std::uint32_t& index) {
...
private:
    bool CustomIndexOf(T const& value, std::uint32_t& index)
    {
        if (m_indexOfFunction)
        {
            ...
            if (delegateIndex != -1)
            {
                index = static_cast<std::uint32_t>(delegateIndex);
                return true;
            }
        }
        return false;
    ...
    winrt::IIterable<winrt::IInspectable>>
{
    ...
    winrt::IInspectable GetAt(std::uint32_t index)
    {
        auto inner = this->GetVectorInnerImpl();
        return inner->GetAt(index);
    }

    bool IndexOf(winrt::IInspectable const& value, std::uint32_t& index)
    {
        auto inner = this->GetVectorInnerImpl();
        return inner->IndexOf(value.as<T>(), index);
    }

    std::uint32_t GetMany(std::uint32_t const startIndex, winrt::array_view<winrt::IInspectable> values)
    {
        throw winrt::hresult_not_implemented();
    }

    void SetAt(std::uint32_t const index, winrt::IInspectable const& value)
    {
        auto inner = this->GetVectorInnerImpl();
    ...
    void InsertAt(std::uint32_t index, winrt::IInspectable const& value)
    {
        auto inner = this->GetVectorInnerImpl();
...
private:
    struct Iterator : winrt::implements<Iterator, winrt::IIterator<winrt::IInspectable>>
    {
        ...
        std::uint32_t GetMany(winrt::array_view<winrt::IInspectable> value)
        {
            throw winrt::hresult_not_implemented();
        }
