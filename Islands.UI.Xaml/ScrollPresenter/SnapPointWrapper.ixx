export module ixx.SnapPointWrapper;
...
private:
    T m_snapPoint;
    std::tuple<double, double> m_actualApplicableZone{ -std::numeric_limits<double>::infinity(), std::numeric_limits<double>::infinity() };
    std::tuple<double, double> m_actualImpulseApplicableZone{ -std::numeric_limits<double>::infinity(), std::numeric_limits<double>::infinity() };
    int m_combinationCount{ 0 };
    double m_ignoredValue{ std::numeric_limits<double>::quiet_NaN() }; // Ignored snapping value when inertia is triggered by an impulse
